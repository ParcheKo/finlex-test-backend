...
public abstract class TestBase
{
    ...
    public static List<T> AssertPublishedDomainEvents<T>(Entity aggregate) where T : IDomainEvent
    {
        ...
        if (!domainEvents.Any()) throw new Exception($"{typeof(T).Name} event not published");
    ...
    public static void AssertBrokenRule<TRule>(TestDelegate testDelegate) where TRule : class, IBusinessRule
    {
        ...
        var businessRuleValidationException = Assert.Catch<BusinessRuleValidationException>(
            testDelegate,
            message
        );
        if (businessRuleValidationException != null)
            Assert.That(
                businessRuleValidationException.BrokenRule,
                Is.TypeOf<TRule>(),
                message
            );
    }
